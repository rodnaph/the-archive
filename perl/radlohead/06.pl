...
if ($q->param('todo') eq 'register') {
  if (userExists($username)) {
    ...
    open_tr();
    start_box( 'Username Taken' );
    print_text( 'Sorry, but the username ' );
    print_user( $username );
    print_text( ' has already been taken.' );
    end_box();
...
EOT
      ...
      print_box( 'Profile Registered', $text );
...
EOT
  ...
  print_text("to register your username here just fill out the form below and click ");
  print_submit("regiSTer");

  start_form("06.pl","return checkForm(this)");
